...
The data set is released in two pieces. The first is the raw output, given in
the "raw" directory; the second, in the "processed" directory, is a derivitive
data set which has been pre-processed from the raw data set. For more details,
see below.
...
Each zpaq archive is a directory with ~2,500 files (raw) or ~20 files
(processed). The SHA1 checksums for each interval are provided in raw.sha1.txt
and processed.sha1.txt.
